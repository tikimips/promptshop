...
export async function POST(req: Request) {
  ...
  let body: { imageB64?: string; imagesB64?: string[]; gallery?: string; notes?: string; poseB64?: string; site?: string };
  try {
    body = await req.json();
  } catch {
  ...
  if (hasPose) parts.push({ inline_data: { mime_type: 'image/jpeg', data: String(body.poseB64) } });
  ...
  for (const model of MODELS) {
    try {
      ...
      for (const p of ps) {
        ...
        if (d) {
          await saveToGallery(d, 'superhero', bucketOf(body)).catch(() => {});
